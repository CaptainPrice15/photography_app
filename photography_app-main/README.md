...
- **FastAPI** backend with auto-migration on startup
...
```bash
cd backend
docker build -t photoexhibit-api .
docker run -p 8000:8000 --env-file .env photoexhibit-api
```
...
```bash
cd frontend
docker build -t photoexhibit-frontend .
docker run -p 3000:3000 photoexhibit-frontend
```
...
The frontend includes a `vercel.json` configuration. Deploy by connecting your GitHub repo to Vercel:
...
The backend includes a `render.yaml` blueprint for Render's Infrastructure-as-Code. Deploy by connecting your GitHub repo to Render and using the Blueprint.
